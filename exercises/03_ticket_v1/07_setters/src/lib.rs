...
fn validate_title(title: &String) {
    match title.as_bytes().len() {
        0 => panic!("Title cannot be empty"),
        len if len > 50 => panic!("Title cannot be longer than 50 characters"),
        _ => (),
    };
}

fn validate_description(description: &String) {
    match description.as_bytes().len() {
        0 => panic!("Description cannot be empty"),
        len if len > 500 => panic!("Description cannot be longer than 500 characters"),
        _ => (),
    }
}
...
fn validate_status(status: &String) {
    match status.as_str() {
        "To-Do" | "In Progress" | "Done" => status.clone(),
